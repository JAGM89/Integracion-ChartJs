...
app.get('/valor-maximo-planificador', async(req,resp)=>{
    try {
        const resultado=await Producto.findAll({
            attributes:['plannerCode',
                [sequelize.fn('MAX',sequelize.col('value')),'ValorMAX']
            ],
            group:['plannerCode']
        });
        if (resultado.length>0) {
...
app.get('/total-marca', async(req,resp)=>{
    try {
        const resultado=await Producto.findAll({
            attributes:['brandCode',
                [sequelize.fn('SUM',sequelize.col('value')),'TotalbrandCode']
            ],
            group:['brandCode']
        });
        if (resultado.length>0) {
...
app.get('/marca-zona', async(req,resp)=>{
    try {
        const resultado=await Producto.findAll({
            attributes:['linecode',
                [sequelize.fn('AVG',sequelize.col('value')),'TotalbrandCode'],
                [sequelize.fn('COUNT',sequelize.col('*')),'CantidadxLinea']
            ],
            group:['linecode']
        });
        if (resultado.length>0) {
